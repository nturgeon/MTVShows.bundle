...
def ShowSearch(query):
    ...
    for item in html.xpath('//div[contains(@class,"mtvn-item-content")]'):
        ...
        if 'www.mtv.com/shows/' not in link:
            continue
        # SHOWS ARE IN TWO FORMATS THE OLD FORMAT ENDS WITH SERIES.JHTML
...
def VideoPage(url, title, page=1):
    ...
    for item in data.xpath('//ol/li[@itemtype="http://schema.org/VideoObject"]'):
        ...
        seas_ep = SeasEpFind(episode='--', title=video_title, season=0)
        episode = int(seas_ep[0])
...
def SeasEpFind(episode, title, season):
    ...
    try:
        ...
        try:
            (episode, season) = RE_SEASON_EP.search(title).groups()
        except:
            (episode, season) = (0, 0)
    if season==0:
        try:
